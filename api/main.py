...
def ask_command(
    ...
    settings = get_settings()
    _verify_agent_token(settings.agent_api_token, x_agent_token)
...
def _verify_internal_request(
    ...
    if settings.agent_api_token and actual_token == settings.agent_api_token:
        return
    if _verify_oidc_service_account(settings, authorization):
        return
...
def _verify_oidc_service_account(settings: Settings, authorization: str | None) -> bool:
